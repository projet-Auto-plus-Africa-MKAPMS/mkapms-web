...
## Reprise exhaustive — IMG_1882/1883, lot Employés

Les deux nouvelles captures confirment 58 signalements ; la demande porte toujours sur chaque défaut des cinq captures précédentes. Aucune ligne ne sera considérée comme résolue par la seule évolution du pourcentage.

AdminEmployes.tsx:52 et :275 : filtres de compteurs et retrait des missions raccordés au moteur Workflow/RH existant. La page avait cinq employés, des coordonnées, notes de performance et missions inventés. Le moteur lit maintenant les comptes internes, dossiers RH, congés approuvés et dernières évaluations. Enregistrer modifie réellement le profil dans une transaction, en préservant contrat et rémunération. La direction ne peut pas modifier les profils de direction sans rôle PDG. Le planning est un modèle hebdomadaire explicite avec missions persistées, contrôle des horaires et chevauchements, idempotence, retrait logique et audit ; les données ne sont pas supprimées.

Migration additive 0139 : colonne profile dans hr_records et table hr_weekly_tasks. Les fonctions RH préexistantes restent disponibles. Tests isolés : migration répétable, refus des rôles non autorisés, données réelles, congés/évaluation, persistance, préservation salaire/contrat, rollback sur doublon email, horaires invalides, chevauchements, idempotence et retrait audité. Aucun compte réel ni planning de production n’a été modifié par les tests. Inventaire statique attendu : 58 → 56. Autres défauts des captures toujours à traiter.
