    ...
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      ...
      <div className="px-4 mt-4 grid grid-cols-2 gap-2">{ZONES.map((z, i) => (
        <button key={z} className={`rounded-xl border-2 border-dashed py-8 flex flex-col items-center gap-2 ${i < 5 ? "border-green-300 bg-green-50" : "border-blue-300 bg-blue-50"}`}>
          ...
          <span className="text-[9px] text-[#9CA3AF]">{i < 5 ? "Uploadée" : "Obligatoire"}</span>
        </button>))}</div>
      <div className="mx-4 mt-4 rounded-xl bg-white border border-[#E5E7EB] p-4 flex items-center gap-3"><Video size={16} className="text-blue-600" /><div className="flex-1"><p className="text-sm font-bold text-[#111]">Vidéo (optionnel)</p><p className="text-[10px] text-[#6B7280]">Disponible selon abonnement</p></div><Upload size={14} className="text-[#9CA3AF]" /></div>
