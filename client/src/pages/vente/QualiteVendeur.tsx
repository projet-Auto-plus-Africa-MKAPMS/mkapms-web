...
export default function QualiteVendeur() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      <div className="bg-blue-800 px-4 pt-6 pb-5"><Link to="/vente/resume-vendeur" className="flex items-center gap-1 text-sm text-white/60 mb-2"><ChevronLeft size={14} /> Dashboard</Link><h1 className="text-xl font-black text-white flex items-center gap-2"><Award size={20} /> Qualité Vendeur</h1></div>
      ...
      <div className="px-4 mt-4"><h3 className="text-sm font-bold text-[#111]">Badges</h3><div className="mt-2 flex flex-wrap gap-2">{BADGES.map(b => (<span key={b.label} className={`rounded-full px-3 py-1 text-xs font-bold ${b.ok ? "bg-blue-600 text-white" : "bg-[#E5E7EB] text-[#9CA3AF]"}`}>{b.label}</span>))}</div></div>
    </div>
  );
}
