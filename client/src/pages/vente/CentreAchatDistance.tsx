  ...
  return (
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      ...
      <div className="mx-4 mt-4 rounded-xl bg-white border border-[#E5E7EB] p-4">{ETAPES.map((e, i) => { const Icon = e.icon; return (
        <div key={i} className="flex items-start gap-3"><div className="flex flex-col items-center"><div className={`h-8 w-8 rounded-full flex items-center justify-center ${e.done ? "bg-[#D4AF37]" : "bg-[#E5E7EB]"}`}><Icon size={14} className={e.done ? "text-white" : "text-[#9CA3AF]"} /></div>{i < ETAPES.length - 1 && <div className={`w-0.5 h-6 ${e.done ? "bg-[#D4AF37]" : "bg-[#E5E7EB]"}`} />}</div><p className={`text-sm pb-4 ${e.done ? "font-bold text-[#111]" : "text-[#9CA3AF]"}`}>{e.label}</p></div>); })}</div>
