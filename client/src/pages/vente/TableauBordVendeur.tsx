...
export default function TableauBordVendeur() {
  return (
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      <div className="bg-blue-800 px-4 pt-6 pb-5"><Link to="/vente/resume-vendeur" className="flex items-center gap-1 text-sm text-white/60 mb-2"><ChevronLeft size={14} /> Pro</Link><h1 className="text-xl font-black text-white flex items-center gap-2"><BarChart3 size={20} /> Résumé Vendeur</h1></div>
      ...
      <div className="px-4 mt-4 space-y-1.5">{MENU.map(m => (<Link key={m.label} to={m.to} className="flex items-center justify-between rounded-xl bg-white border border-[#E5E7EB] p-3"><span className="text-sm font-semibold text-[#111]">{m.label}</span><ChevronRight size={14} className="text-red-500" /></Link>))}</div>
    </div>
  );
}
