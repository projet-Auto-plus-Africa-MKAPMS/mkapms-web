    ...
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      ...
      <div className="px-4 mt-4 space-y-2">{DOCS_CLIENT.map((d) => (
        <div key={d.label} className="rounded-xl bg-white border border-[#E5E7EB] p-3 flex items-center gap-3">
          <FileText size={14} className={d.statut === "valide" ? "text-green-600" : d.statut === "en_cours" ? "text-amber-500" : "text-red-500"} />
          <span className="flex-1 text-sm text-[#111]">{d.label}</span>
          <span className={`rounded-full px-2 py-0.5 text-[9px] font-bold ${d.statut === "valide" ? "bg-green-50 text-green-600" : d.statut === "en_cours" ? "bg-amber-50 text-amber-600" : "bg-gray-50 text-red-500"}`}>{d.statut === "valide" ? "Validé" : d.statut === "en_cours" ? "En cours" : "En attente"}</span>
