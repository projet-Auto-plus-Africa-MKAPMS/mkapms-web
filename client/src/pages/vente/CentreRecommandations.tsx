    ...
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      ...
      <div className="px-4 mt-4 space-y-4">{SECTIONS.map(s => (
        <div key={s.titre}><h3 className="text-sm font-bold text-[#111] mb-2">{s.titre}</h3>{s.items.map(it => (
          <div key={it.nom} className="rounded-xl bg-white border border-[#E5E7EB] p-3 mb-1.5 flex items-center gap-3"><div className="flex-1"><h4 className="text-sm font-semibold text-[#111]">{it.nom}</h4></div><span className="text-sm font-bold text-[#D4AF37]">{it.prix}</span><ChevronRight size={16} className="text-red-500" /></div>))}</div>))}</div>
    </div>
  );
}
