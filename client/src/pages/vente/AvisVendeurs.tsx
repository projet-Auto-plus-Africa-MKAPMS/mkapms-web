...
export default function AvisVendeurs() {
  return (
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      <div className="bg-blue-800 px-4 pt-6 pb-5"><Link to="/vente/resume-vendeur" className="flex items-center gap-1 text-sm text-white/60 mb-2"><ChevronLeft size={14} /> Dashboard</Link><h1 className="text-xl font-black text-white flex items-center gap-2"><Star size={20} /> Avis Vendeurs</h1></div>
      ...
      <div className="px-4 mt-4 space-y-2">{AVIS.map((a, i) => (<div key={i} className="rounded-xl bg-white border border-[#E5E7EB] p-4"><div className="flex justify-between"><span className="text-sm font-bold text-[#111]">{a.client}</span><div className="flex gap-0.5">{Array.from({length: a.note}, (_, n) => <Star key={n} size={10} className="text-[#D4AF37]" fill="#D4AF37" />)}</div></div><p className="text-xs text-[#6B7280] mt-1">{a.texte}</p><p className="text-[9px] text-[#9CA3AF] mt-1">{a.date}</p></div>))}</div>
    </div>
  );
}
