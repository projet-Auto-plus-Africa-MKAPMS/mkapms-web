...
export default function LocationUtilitaires() {
  ...
  const realAnnonces = trpc.annonces.list.useQuery(
    { type: "location", categorie: "utilitaire", ville: rechercheActive && lieu ? lieu : undefined, limit: 24 },
    { retry: false },
  );
  ...
  return (
    <div className="min-h-screen bg-[#F5F3EF] pb-24 max-w-6xl mx-auto">
        ...
        <div className="space-y-3">
          ...
          <button
            onClick={() => {
              ...
              document.getElementById("resultats-utilitaires")?.scrollIntoView({ behavior: "smooth" });
            }}
      ...
      {rechercheActive && (
        <div id="resultats-utilitaires" className="px-4 mt-6">
          ...
          {annoncesTrouvees.length > 0 ? (
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {annoncesTrouvees.map((a) => (
                  <Link key={a.id} to={`/louer/utilitaires/vehicule/${a.id}`} className="block rounded-xl bg-white border border-[#111] overflow-hidden active:scale-[0.99] transition hover:shadow-lg">
            ...
            <p className="rounded-xl bg-white border border-[#E5E7EB] p-4 text-center text-xs text-[#6B7280]">
              Aucune annonce réelle d'utilitaire ne correspond à cette recherche pour le moment — le catalogue ci-dessous reste consultable.
            </p>
          )}
        </div>
      )}
