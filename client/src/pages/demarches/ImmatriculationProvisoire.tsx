...
/** Données réelles : trpc.carteGrise.createDossier (type "ww_cpi" — Certificat Provisoire d'Immatriculation, table cg_dossiers). */
export default function ImmatriculationProvisoire() {
  ...
  const mesDemandes = (mesDossiers.data ?? []).filter((d) => d.type === "ww_cpi");
  ...
  const create = trpc.carteGrise.createDossier.useMutation({
    onSuccess: async (dossier) => {
      ...
      setImmatriculation(""); setVin(""); setDocuments([]);
  ...
  return (
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      ...
      <form
        onSubmit={(e) => {
          e.preventDefault();
          if (!immatriculation.trim() && !vin.trim()) return;
          create.mutate({ type: "ww_cpi", immatriculation: immatriculation || undefined, vin: vin || undefined });
        }}
        className="mx-4 mt-4 rounded-xl bg-white border border-[#E5E7EB] p-4 space-y-3"
      >
        <div><label className="text-xs text-[#6B7280]">Plaque / VIN</label><input value={immatriculation || vin} onChange={(e) => { const v = e.target.value; if (v.length === 17) { setVin(v); setImmatriculation(""); } else { setImmatriculation(v); setVin(""); } }} placeholder="AB-123-CD ou VIN" className="mt-1 w-full rounded-lg border border-[#E5E7EB] px-3 py-2.5 text-sm" /></div>
        <div>
          <label className="text-xs text-[#6B7280]">Carte grise étrangère ou ancienne, pièce d'identité, justificatif domicile, assurance</label>
          <div className="mt-1"><FileUpload compact label="Télécharger" accept="image/*,.pdf" onUploaded={(files) => setDocuments((p) => [...p, ...files])} /></div>
        </div>
        <button type="submit" disabled={create.isPending || (!immatriculation.trim() && !vin.trim())} className="w-full rounded-xl bg-purple-700 py-3 text-sm font-bold text-white active:scale-[0.98] disabled:opacity-50">
          {create.isPending ? "Envoi…" : "Demander WW provisoire"}
        </button>
