  ...
  return (
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      ...
      <div className="px-4 mt-4 space-y-2">{ARCHIVES.map(a => (
        <div key={a.label} className="rounded-xl bg-white border border-[#E5E7EB] p-3 flex items-center gap-3"><FileText size={14} className="text-[#D4AF37]" /><div className="flex-1"><h3 className="text-sm text-[#111]">{a.label}</h3><p className="text-[9px] text-[#6B7280]">{a.type} · {a.date}</p></div><Download size={14} className="text-[#9CA3AF]" /></div>))}</div>
    </div>
  );
}
