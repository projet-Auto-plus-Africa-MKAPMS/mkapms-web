  ...
  return (
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      ...
      <div className="px-4 mt-4 space-y-2">{SERVICES.map(s => (
        <Link key={s.label} to="/demarches/suivi-dossier" className="flex items-center gap-3 rounded-xl bg-white border border-[#E5E7EB] p-4 active:scale-[0.99]"><div className="flex-1"><h3 className="text-sm font-bold text-[#111]">{s.label}</h3><p className="text-[10px] text-[#6B7280]">{s.desc}</p></div><ChevronRight size={14} className="text-[#D4D4D4]" /></Link>))}</div>
    </div>
  );
}
