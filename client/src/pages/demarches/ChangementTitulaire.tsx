...
/** Données réelles : trpc.carteGrise.createDossier (type "changement_titulaire", table cg_dossiers). */
export default function ChangementTitulaire() {
  ...
  const mesDemandes = (mesDossiers.data ?? []).filter((d) => d.type === "changement_titulaire");
    ...
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      ...
      <form
        onSubmit={(e) => {
          ...
          create.mutate({ type: "changement_titulaire", immatriculation, vendeurNom: vendeurNom || undefined, acheteurNom: acheteurNom || undefined });
        }}
      ...
      >
        ...
        <div><label className="text-xs text-[#6B7280]">Ancien titulaire</label><input value={vendeurNom} onChange={(e) => setVendeurNom(e.target.value)} className="mt-1 w-full rounded-lg border border-[#E5E7EB] px-3 py-2.5 text-sm" /></div>
        <div><label className="text-xs text-[#6B7280]">Nouveau titulaire</label><input value={acheteurNom} onChange={(e) => setAcheteurNom(e.target.value)} className="mt-1 w-full rounded-lg border border-[#E5E7EB] px-3 py-2.5 text-sm" /></div>
        <div>
          <label className="text-xs text-[#6B7280]">Pièces (carte grise, pièce d'identité, justificatif domicile, certificat de cession)</label>
          <div className="mt-1"><FileUpload compact label="Télécharger" accept="image/*,.pdf" onUploaded={(files) => setDocuments((p) => [...p, ...files])} /></div>
        </div>
        <button type="submit" disabled={create.isPending || !immatriculation.trim()} className="w-full rounded-xl bg-blue-700 py-3 text-sm font-bold text-white active:scale-[0.98] disabled:opacity-50">
