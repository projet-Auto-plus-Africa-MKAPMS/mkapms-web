      ...
      <div className="px-4 mt-4 space-y-3">{DOSSIERS.map(d => (
        <div key={d.or} className="rounded-xl bg-white border border-[#E5E7EB] p-4">
          ...
          <div className="mt-2 grid grid-cols-3 gap-2 text-center">{[{l:"Avant",n:d.avant},{l:"Pendant",n:d.pendant},{l:"Après",n:d.apres}].map(p => (
            <div key={p.l} className={`rounded-lg py-3 ${p.n > 0 ? "bg-[#D4AF37]/10 border border-[#D4AF37]/30" : "border-2 border-dashed border-[#E5E7EB]"}`}><Camera size={14} className={`mx-auto ${p.n > 0 ? "text-[#D4AF37]" : "text-red-500"}`} /><p className="text-[9px] font-bold mt-1">{p.l}: {p.n}</p></div>))}</div>
        </div>))}</div>
    </div>
  );
}
