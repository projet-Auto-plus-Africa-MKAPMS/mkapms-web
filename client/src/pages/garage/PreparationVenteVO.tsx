  ...
  return (
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      ...
      <div className="px-4 mt-3 space-y-1.5">{CHECKLIST.map(c => (
        <div key={c.label} className="rounded-xl bg-white border border-[#E5E7EB] p-3 flex items-center gap-3">{c.ok ? <Check size={14} className="text-green-600" /> : <X size={14} className="text-[#9CA3AF]" />}<span className="text-sm text-[#111]">{c.label}</span></div>))}</div>
