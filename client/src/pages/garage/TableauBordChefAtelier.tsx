...
export default function TableauBordChefAtelier() {
  return (
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      ...
      <div className="px-4 mt-3 space-y-1.5">{MENU.map(m => (<Link key={m.label} to={m.to} className="flex items-center justify-between rounded-xl bg-white border border-[#E5E7EB] p-3"><span className="text-sm font-semibold text-[#111]">{m.label}</span><ChevronRight size={14} className="text-red-500" /></Link>))}</div>
    </div>
  );
}
