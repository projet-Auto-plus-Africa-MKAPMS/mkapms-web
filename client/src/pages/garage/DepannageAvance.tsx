  ...
  return (
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      ...
      <div className="mx-4 mt-4 rounded-xl bg-white border border-[#E5E7EB] p-4">{SUIVI.map((s, i) => (
        <div key={i} className="flex items-start gap-3"><div className="flex flex-col items-center"><div className={`h-6 w-6 rounded-full flex items-center justify-center text-[10px] font-bold ${s.fait ? "bg-red-600 text-white" : "bg-[#E5E7EB] text-[#9CA3AF]"}`}>{i + 1}</div>{i < SUIVI.length - 1 && <div className={`w-0.5 h-4 ${s.fait ? "bg-red-400" : "bg-[#E5E7EB]"}`} />}</div><div className="pb-2"><p className={`text-sm ${s.fait ? "font-bold text-[#111]" : "text-[#9CA3AF]"}`}>{s.label}</p>{s.heure && <p className="text-[9px] text-[#9CA3AF]">{s.heure}</p>}</div></div>))}</div>
