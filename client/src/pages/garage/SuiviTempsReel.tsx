  ...
  return (
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      ...
      <div className="mx-4 mt-4 rounded-xl bg-white border border-[#E5E7EB] p-4">{STATUTS.map((s, i) => { const Icon = s.icon; return (
        <div key={i} className="flex items-start gap-3"><div className="flex flex-col items-center"><div className={`h-8 w-8 rounded-full flex items-center justify-center ${s.fait ? "bg-[#D4AF37]" : "bg-[#E5E7EB]"}`}><Icon size={14} className={s.fait ? "text-white" : "text-red-500"} /></div>{i < STATUTS.length - 1 && <div className={`w-0.5 h-8 ${s.fait ? "bg-[#D4AF37]" : "bg-[#E5E7EB]"}`} />}</div><div className="pb-4"><p className={`text-sm ${s.fait ? "font-bold text-[#111]" : "text-red-500"}`}>{s.label}</p>{s.date && <p className="text-[9px] text-red-500">{s.date}</p>}</div></div>); })}</div>
    </div>
  );
}
