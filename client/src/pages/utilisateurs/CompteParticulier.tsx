  ...
  return (
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      ...
      <div className="px-4 -mt-3 relative z-10 space-y-1.5">{MENUS.map(m => { const Icon = m.icon; return (
        <Link key={m.label} to={m.to} className="flex items-center gap-3 rounded-xl bg-white border border-[#E5E7EB] p-3 active:scale-[0.99]"><Icon size={16} className="text-[#D4AF37]" /><span className="flex-1 text-sm font-semibold text-[#111]">{m.label}</span><ChevronRight size={14} className="text-[#D4D4D4]" /></Link>); })}</div>
    </div>
  );
}
