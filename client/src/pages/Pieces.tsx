...
export default function Pieces() {
  ...
  /**
   * Ouvre l'écran de paiement réel d'une commande.
   * URL du prestataire → sortie du site ; chemin interne → navigation SPA.
   */
  const ouvrirPaiement = async (orderId: number) => {
   ...
   *
   * La commande créée est immédiatement suivie de l'ouverture du paiement : le
   * panier n'est vidé qu'une fois la commande réellement enregistrée, et un
   * échec de paiement laisse la commande consultable au lieu de la perdre.
   */
        ...
        <div className="mt-4 rounded-xl border border-gold/30 bg-gold-soft/30 p-5">
          ...
          {cart.length === 0 ? (
            ...
            <>
              ...
              {erreurPaiement && (
                <p className="mt-2 text-xs font-semibold text-danger">{erreurPaiement}</p>
              )}
              ...
              return (
                <div key={o.id} className="card p-5">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    {o.status === "panier" && (
                      <button
                        onClick={() => ouvrirPaiement(o.id)}
                        disabled={payOrder.isPending}
                        className="btn-acheter !py-2 !text-xs disabled:opacity-50"
                      >
                        <CreditCard size={13} className="mr-1 inline" /> Payer par carte
                      </button>
                    )}
                  </div>
                </div>
