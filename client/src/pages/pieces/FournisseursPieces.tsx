  ...
  return (
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      ...
      <div className="px-4 mt-4 space-y-2">{FOURNISSEURS.map(f => (
        <div key={f.nom} className="rounded-xl bg-white border border-[#E5E7EB] p-3 flex items-center gap-3"><div className="flex-1"><h3 className="text-sm font-bold text-[#111]">{f.nom}</h3><p className="text-[9px] text-[#6B7280]">{f.type} · {f.produits} réf.</p></div><ChevronRight size={14} className="text-[#D4D4D4]" /></div>))}</div>
    </div>
  );
}
