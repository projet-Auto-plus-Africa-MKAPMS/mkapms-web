...
export default function PiecesHuiles() {
  return (
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      ...
      <div className="px-4 mt-4 space-y-1.5">{ITEMS.map(i => (<Link key={i} to="/pieces/recherche" className="flex items-center justify-between rounded-xl bg-white border border-[#E5E7EB] p-3 active:scale-[0.99]"><span className="text-sm font-semibold text-[#111]">{i}</span><ChevronRight size={16} className="text-red-500" /></Link>))}</div>
    </div>
  );
}
