  ...
  return (
    <div className="min-h-screen bg-[#F5F3EF] pb-24">
      ...
      <div className="mx-4 mt-4 rounded-xl bg-white border border-[#E5E7EB] p-4 space-y-3">{PANIER.map(p => (
        <div key={p.label} className="flex items-center gap-3 py-2 border-b border-[#F3F4F6]"><div className="flex-1"><h3 className="text-sm text-[#111]">{p.label}</h3>{p.montage && <p className="text-[9px] text-[#D4AF37] flex items-center gap-0.5"><Wrench size={8} /> + Montage {p.montageP} €</p>}</div><span className="text-sm font-bold">{p.prix + (p.montage ? p.montageP : 0)} €</span><Trash2 size={14} className="text-red-500" /></div>))}
