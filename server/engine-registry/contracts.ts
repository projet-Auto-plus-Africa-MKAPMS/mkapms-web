...
export const ENGINE_CONTRACTS: EngineContract[] = [
  ...
  },
  {
    ...
    category: "service",
    description:
      "Capacités serveur de l'atelier : validation interne, contrôle qualité, stock de pièces du garage, réapprovisionnement gouverné (seuil → proposition → décision humaine → commande fournisseur sous plafond → réception), report de rendez-vous.",
    responsibilities: [
      ...
      "Ouvrir une proposition de réapprovisionnement persistante dès qu'une ligne passe sous son seuil (idempotente par ligne).",
      "Exiger une décision humaine tracée (validation ou refus motivé) avant toute commande.",
      "Passer la commande fournisseur sous le plafond mensuel du garage, refuser au-delà, et dire si le bon est réellement parti.",
      "Incrémenter le stock à la réception avec un mouvement par ligne au nom de la commande.",
    ...
    permissions: [
      ...
      {
        key: "atelier.reappro.decide",
        kind: "endpoint",
        roles: ["pro", "super_admin"],
        description: "Valider ou refuser une proposition de réapprovisionnement d'un garage possédé.",
      },
      {
        key: "atelier.reappro.commander",
        kind: "endpoint",
        roles: ["pro", "super_admin"],
        description: "Engager financièrement le garage (commande fournisseur) sous son plafond mensuel.",
      },
    ],
