...
async function http(chemin: string): Promise<
  { ok: true; status: number; corps: string } | { ok: false; motif: string; reseau: boolean }
> {
  const b = base();
  if (!b) {
    return {
  ...
  try {
    const res = await fetch(`${b}${chemin}`, { redirect: "follow" });
...
async function scalaire(requete: string): Promise<number | null> {
  try {
