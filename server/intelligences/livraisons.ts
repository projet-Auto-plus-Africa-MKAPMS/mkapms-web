...
export const LIVRAISONS: Livraison[] = [
  ...
  },
  {
    ...
    titre: "Chat IA toujours en panne après le correctif des noms d'outils : une DEUXIÈME erreur HTTP 400 distincte, propre aux modèles de raisonnement",
    moteurs: ["intelligences"],
    quoi:
      "Signalé par le PDG juste après le déploiement du correctif des noms d'outils (livraison « outils-nom-fournisseur-points-interdits-openai ») : le chat MKA.P-MS AI restait « ne fonctionne pas du tout », avec cette fois un message OpenAI totalement différent : « Function tools with reasoning_effort are not supported for gpt-5.6-sol in /v1/chat/completions. To use function tools, use /v1/responses or set reasoning_effort to 'none'. » Audit de server/intelligences/provider.ts (`appeler()`, seul point d'appel réel au fournisseur) : le corps de requête n'envoyait jamais `reasoning_effort` — le modèle de raisonnement réellement découvert sur le compte (résolution dynamique via /v1/models, jamais un nom codé en dur ici) le refuse par défaut dès que des outils sont présents sur /v1/chat/completions.\n\nCorrigé sans jamais deviner à l'avance quel modèle serait concerné : le premier appel reste inchangé (aucun reasoning_effort ajouté par défaut) ; si — et seulement si — le fournisseur renvoie précisément cette erreur réelle (« reasoning_effort » et « tools » tous deux présents dans le message, avec des outils réellement envoyés dans la requête), l'appel est rejoué une seule fois avec `reasoning_effort: \"none\"` ajouté, exactement le remède documenté par OpenAI lui-même dans son propre message d'erreur. Aucune boucle possible : un seul rejeu, jamais plus.",
    pourquoi:
      "Le premier correctif (noms d'outils) était nécessaire mais pas suffisant : il a permis à l'appel d'atteindre enfin le fournisseur avec des outils syntaxiquement valides, ce qui a révélé une seconde incompatibilité, jusque-là masquée par la première. Deviner à l'avance la liste des modèles de raisonnement concernés aurait été fragile (un nouveau modèle découvert demain porterait un nom différent) : la seule source fiable est l'erreur réelle renvoyée par le fournisseur au moment où elle se produit.",
    ou: ["server/intelligences/provider.ts", "server/intelligences/__tests__/reasoning-effort-outils.test.ts"],
    lecon:
      "Vérifié par test réel (server/intelligences/__tests__/reasoning-effort-outils.test.ts, 15/15, appeler() réel de production, seul fetch injecté) : (1) outils + erreur réelle « reasoning_effort » → exactement deux tentatives HTTP, la première sans reasoning_effort, la seconde avec reasoning_effort:\"none\" et les outils toujours présents, appel d'outil du modèle bien transmis ; (2) même erreur mais aucun outil réellement envoyé → aucun rejeu, échec honnête rapporté tel quel ; (3) une erreur 400 sans rapport avec reasoning_effort → aucun rejeu hors-sujet ; (4) le rejeu échoue aussi → exactement deux tentatives, jamais une boucle, échec final réel. npx tsc --noEmit : aucune erreur. Suites déjà existantes rejouées sans régression après réparation des tables ai-fabric manquantes en base de test locale (même écart de migrations pré-existant que la tâche #61, sans lien avec ce correctif) : fuite-fournisseurs (39/39), independance-openai (10/10), boucle (333/333), merchant-center (22/22), fondations (12/12). npm run build à confirmer avant fusion. Pas de vérification visuelle en production à ce stade.\n\nLeçon générale : un correctif qui répare la première erreur visible peut simplement démasquer la suivante — surtout quand la fonctionnalité touchée (ici : appeler l'IA avec des outils) n'avait jamais réellement fonctionné en production jusque-là. Toujours revérifier avec le PDG après un correctif « critique » plutôt que supposer la panne résolue tant que le scénario réel (taper un message dans le chat) n'a pas été confirmé.",
    domaine: "moteurs",
  },
];
