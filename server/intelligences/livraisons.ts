...
export const LIVRAISONS: Livraison[] = [
  ...
  {
    cle: "product-engine-compteur-vehicules-colonne-inexistante",
    titre: "CentreProduitsGoogle.tsx : le compteur « Véhicules » affichait toujours 0 — la requête interrogeait une colonne inexistante, l'erreur était silencieusement absorbée",
    moteurs: ["product_engine"],
    quoi:
      "Signalé par le PDG (capture d'écran) : sur l'écran « Produits Google & Merchant », les deux tuyaux affichaient « 0 0 » et il a demandé de vérifier pourquoi rien ne se remplit jamais, en distinguant explicitement ce qui peut être normal (pages produits fictives) de ce qui l'inquiète (un tuyau entier resté à zéro).\n\nLecture de pipelinesSnapshot() (server/product-engine/service.ts) : le compteur « Véhicules » exécutait `SELECT count(*) FROM annonces WHERE statut = 'publiee'` — la colonne s'appelle en réalité `status` (vérifié contre server/schema.ts, confirmé à de nombreuses reprises dans ce dépôt). Cette requête levait donc systématiquement une erreur SQL, absorbée silencieusement par un bloc try/catch qui retombait sur `vehicules = 0` sans jamais rien signaler — un bug qui produit exactement 0 quel que soit le nombre réel d'annonces publiées, indiscernable d'un vrai zéro. Corrigé en remplaçant le SQL brut par une requête drizzle typée (`eq(annonces.status, \"publiee\")`) : une erreur de nom de colonne serait désormais une erreur de compilation, pas une erreur silencieuse en production.\n\nLe tuyau « Produits / pièces » a été vérifié séparément et est correct : sa requête interroge productFeedItems (alimentée par un vrai parcours parts_catalog → syncProduit, colonnes confirmées une à une contre le schéma réel). Un 0 sur ce tuyau, une fois ce correctif déployé, signifie soit qu'aucune vraie pièce active n'existe encore dans parts_catalog, soit que « Relire » n'a jamais été exécuté depuis le dépôt des pièces existantes — deux situations réelles, jamais un bug caché comme celui du tuyau véhicules, et qu'aucune fabrication de données de démonstration n'aurait le droit de masquer.",
    pourquoi:
      "Une erreur SQL absorbée en silence et retombant sur zéro est la pire forme de fabrication involontaire : elle ne ment pas activement, mais elle rend un vrai zéro et un compteur cassé strictement indiscernables, empêchant la Direction de distinguer « aucune annonce publiée » de « le calcul ne fonctionne pas ». Remplacer le SQL brut par une requête typée transforme cette classe d'erreur en échec de compilation, détecté avant tout déploiement plutôt que masqué après.",
    ou: ["server/product-engine/service.ts", "server/product-engine/__tests__/pipelines-snapshot.test.ts"],
    lecon:
      "Vérifié directement contre la base réelle (server/product-engine/__tests__/pipelines-snapshot.test.ts, 6/6, écrit à cette occasion — aucun test n'existait avant pour ce moteur) : une vraie annonce publiée fait passer le compteur de N à N+1 (la requête cassée renvoyait toujours 0, quel que soit N) ; une annonce en brouillon n'est jamais comptée ; une vraie pièce insérée dans parts_catalog est bien examinée et synchronisée par refreshFeed() jusque dans productFeedItems avec son vrai nom. En vérifiant l'environnement de test local pour écrire ce test, découvert que la migration 0086 (tables product_feed_items/product_sync_events/product_feed_runs) n'y avait jamais été appliquée alors que 141 migrations plus récentes l'étaient toutes — signe possible de la chaîne de snapshots divergente déjà trackée en tâche #61, à surveiller si le même écart existe ailleurs. npx tsc --noEmit : aucune erreur nouvelle (44 pré-existantes, inchangées). npm run build vert de bout en bout.\n\nCette investigation fait suite à une plainte légitime du PDG sur le fait que des corrections précédentes (carte Propositions) n'avaient pas d'effet visible perçu — ici, la vérification a été poussée jusqu'à la vraie base de données plutôt que de se limiter à relire le code, ce qui a permis de trouver un bug réel et vérifiable plutôt qu'une simple confusion de navigation.",
    domaine: "confiance",
  },
  {
    cle: "country-os-capacites-google-par-pays",
    titre: "Country OS : registre de capacités Google par pays, jamais inventé — fondation de la doctrine PDG v1.1 (architecture mondiale)",
    moteurs: ["country_os"],
    quoi:
      "Le PDG a corrigé la doctrine (ajout §12 de mkapms-mos-architecture.md, v1.1) : interdiction absolue de coder `country = FR` en dur dans l'architecture Google/campagnes, et obligation de disposer d'un CountryGoogleCapabilities consultable par tout moteur avant d'activer une surface Google dans un pays. Tâche #65 (première d'un plan à 8 lots #65-#72).\n\nAudit préalable : server/country-os/index.ts existe déjà, réel et branché (`country.*`, 9 procédures, 20 pays seedés, testé par country-language-app-router.test.ts) — c'est le CountryRegistry demandé, pas à reconstruire. server/country-policy/ (cpe_rules/cpe_evaluations) est déjà le CountryComplianceRules demandé (règle jamais permissive par défaut). La seule pièce manquante identifiée : aucune table ne dit, par pays, quelles surfaces Google (Search, Merchant, Free Listings, Shopping, Vehicle Ads, Google Business, Local Ads) sont officiellement disponibles — exactement le trou qui avait produit le bug du tuyau véhicules (0/0) dans le lot précédent.\n\nConstruit : table `country_google_capabilities` dans le même fichier country-os/index.ts (schéma déjà établi de ce moteur, pas de nouvelle arborescence). Une capacité ne peut passer à `verified=true` sans `sourceRef` : `upsertGoogleCapabilities()` refuse explicitement toute activation sans preuve, avec un message qui nomme l'interdiction. `isGoogleCapabilityEligible()` est le point d'appel unique que les moteurs Google (#66-72) doivent utiliser au lieu d'écrire une liste de pays en dur — absence de ligne ou `verified=false` vaut toujours « indisponible ». Endpoints `country.google.list/get/isEligible` (lecture ouverte) et `country.google.upsert` (PDG uniquement, même sensibilité que country-policy).\n\nSeed réel (migration 0144, additive) : recherché les pages officielles support.google.com (Shopping ads/free listings answer/160637 ; Vehicle Ads answer/11189169, /17003740, /14933131) au 25/09/2026 et rempli uniquement les 11 pays de nos 20 déjà seedés qui y sont explicitement nommés (FR, BE, CH, DE, ES, IT, PT, GB, CA, US, AE) — chacun avec sa sourceRef exacte. Les 9 autres (LU, CI, SN, CM, ML, BF, MA, DZ, TN) n'ont volontairement reçu aucune ligne : absents de la liste trouvée, donc non vérifiés, jamais supposés « non éligibles » ni « éligibles » par défaut. Vehicle Ads correctement daté : FR depuis août 2025, DE/ES/IT depuis mars 2026, GB depuis octobre 2024, CA/US en accès direct — search est vrai par défaut pour tous (SEO organique universel, sans restriction pays connue).\n\nDécouverte annexe (hors périmètre de #65, trackée séparément en #73) : client/src/pages/GlobalCountryEngine.tsx — l'écran « Global Country Engine » censé piloter exactement ce moteur — est 100% fabriqué en état React local (INITIAL_COUNTRIES codé en dur, stats inventées comme ca: 2340000€ pour la France), sans aucun appel tRPC ; il ignore complètement le vrai country-os déjà fonctionnel depuis le Sprint 3. Non touché ici pour ne pas dévier le lot en cours, mais tracké en tâche #73.",
    pourquoi:
      "Le PDG a explicitement rattaché cette correction au bug déjà vécu (compteur Véhicules à 0/0 par colonne SQL inexistante) : une disponibilité Google jamais vérifiée mais supposée vraie, ou jamais vérifiée et supposée fausse, produit la même classe de dégât que l'ancien bug — un chiffre affiché qui ne dit pas la vérité. La règle de fin à 100% (doctrine §12.16-12.18) exige aussi qu'aucun moteur du chantier Google (#66-72) ne puisse être bâti sur une liste de pays inventée en dur, faute de quoi chaque futur moteur reproduirait le même risque.",
    ou: ["mkapms-mos-architecture.md", "server/country-os/index.ts", "drizzle/0144_country_google_capabilities.sql", "server/country-os/__tests__/google-capabilities.test.ts"],
    lecon:
      "Avant de construire, l'audit a évité deux duplications : country-os (CountryRegistry) et country-policy (CountryComplianceRules) existaient déjà, réels et branchés — la doctrine demandait des noms (CountryRegistry, CountryComplianceRules, ...) qui correspondaient déjà à des moteurs livrés sous d'autres noms. Seule la vraie lacune (capacités Google) a été construite, dans le fichier existant plutôt qu'une nouvelle arborescence — règle MOS #15 (conserver, fusionner, renforcer). Vérifié par test réel (server/country-os/__tests__/google-capabilities.test.ts, 11/11) : le seed FR a bien sa sourceRef et son statut vérifié ; un pays sans ligne (jamais un vrai pays existant, une ligne de test 'ZZ') n'a aucune capacité éligible ; upsertGoogleCapabilities() rejette explicitement toute tentative d'activation sans verified+sourceRef, y compris verified=true seul sans sourceRef ; une fois vérifiée avec sourceRef, la capacité déclarée devient immédiatement éligible mais une capacité non déclarée sur ce même pays reste indisponible ; search reste vrai par défaut. npx tsc --noEmit : aucune erreur nouvelle dans country-os (44 erreurs pré-existantes ailleurs, inchangées). npm run check:migrations / check:moteurs / check:boutons / check:cliquables et npm run build : tous verts après régénération de server/data/moteurs.ts. Pas de vérification visuelle en production (écran GlobalCountryEngine non reconnecté, tracké séparément).\n\nLeçon générale pour les lots #66-72 restants : ne jamais coder une liste de pays éligibles en dur dans un moteur Google — toujours appeler isGoogleCapabilityEligible() de ce registre, qui restera la seule source de vérité mise à jour au fil des vérifications PDG.",
    domaine: "moteurs",
  },
  {
    cle: "product-engine-pays-langue-devise-jamais-en-dur",
    titre: "Google Product Engine : chaque fiche produit était forcée pays=FR/langue=fr, quel que soit le vrai pays de la boutique ou du propriétaire",
    moteurs: ["product_engine", "country_os"],
    quoi:
      "Tâche #66 (doctrine PDG v1.1, §12.1 : « ne jamais coder country = FR comme règle métier permanente »). Audit de server/product-engine/service.ts après la fondation Country OS de la tâche #65 : candidatsBoutique() (parts_catalog, boutique pièces pro) et candidatsInventaire() (pieces, inventaire simple) écrivaient littéralement `pays: \"FR\", langue: \"fr\"` sur CHAQUE fiche envoyée au flux Google, sans jamais lire le pays réel de la boutique (`parts_shops.country_code`, déjà une vraie colonne, jamais consultée) ni celui du propriétaire (`users.country`/`users.currency`, déjà de vraies colonnes, jamais consultées). Un vendeur allemand ou anglais aurait donc vu sa fiche signalée à Google Merchant Center comme si elle était française.\n\nCorrigé : `candidatsBoutique()` joint désormais `parts_shops` et lit `country_code` ; `candidatsInventaire()` joint `users` et lit `country`/`currency`. La langue de chaque fiche est dérivée du pays réel via une nouvelle fonction `langueDe()` qui appelle le Country OS déjà construit (`getCountry()`) au lieu d'inventer ou de dupliquer un moteur de langue. `server/product-engine/schema.ts` : suppression des défauts `.default(\"FR\")`/`.default(\"fr\")`/`.default(\"EUR\")` sur `product_feed_items.pays/langue/devise` (migration 0145) — ils n'étaient jamais réellement exercés (le service fournit toujours une valeur) mais restaient une réintroduction possible du bug pour un futur insert qui les omettrait.\n\nDécouverte pendant la construction de `langueDe()` : `country_countries.default_language` (Country OS, tâche #65) avait été seedé à 'fr' pour les 20 pays SANS EXCEPTION (drizzle/0038), y compris l'Allemagne, l'Espagne, l'Italie, le Portugal, le Royaume-Uni, les États-Unis et les pays arabophones — sans cette correction, `langueDe()` aurait silencieusement renvoyé 'fr' pour tout le monde malgré la lecture correcte du pays. Corrigé (migration 0146) pour les 10 pays mono-langue nationale sans ambiguïté (DE→de, ES→es, IT→it, PT→pt, GB→en, US→en, MA/DZ/TN/AE→ar) ; les pays multilingues où le français reste une langue officielle plausible pour cette plateforme francophone (BE, LU, CH, CA, CI, SN, CM, ML, BF) n'ont volontairement pas été touchés — les corriger aurait été un choix éditorial, pas une correction factuelle.\n\nAudit complémentaire : la chaîne de snapshots divergente déjà trackée en tâche #61 touchait aussi le Country OS localement — `country_countries`/`country_currencies` (migration 0038) étaient absentes de la base de test locale malgré des migrations bien plus récentes déjà appliquées. Appliquée manuellement pour pouvoir écrire un test réel ; le même écart pourrait exister ailleurs, comme déjà noté pour la tâche #61.",
    pourquoi:
      "La correction de la tâche #65 (registre de capacités Google jamais inventé) n'aurait aucune valeur si, dans le même moteur, chaque fiche produit continuait d'être envoyée à Google comme si elle venait de France : la disponibilité Google d'un pays devient inutile si l'objet lui-même prétend toujours être français. C'est exactement l'anti-pattern nommé par le PDG dans sa correction de doctrine.",
    ou: [
      "server/product-engine/service.ts",
      "server/product-engine/schema.ts",
      "drizzle/0145_product_feed_items_no_fr_default.sql",
      "drizzle/0146_country_default_language_correction.sql",
      "server/product-engine/__tests__/no-hardcoded-country.test.ts",
    ],
    lecon:
      "Vérifié par test réel (server/product-engine/__tests__/no-hardcoded-country.test.ts, 4/4) : une boutique déclarée en Allemagne produit une fiche pays=DE/langue=de (jamais FR/fr) ; un propriétaire déclaré au Royaume-Uni produit une fiche pays=GB/langue=en/devise=GBP lue depuis users.country/currency, jamais un défaut EUR/FR en dur. Les tests de la tâche #65 (11/11) et de la fondation véhicules (6/6) ont été rejoués sans régression. npx tsc --noEmit : aucune erreur nouvelle. npm run check:migrations/check:moteurs (régénéré)/check:boutons/check:cliquables et npm run build : tous verts.\n\nLeçon générale : une correction de doctrine sur un moteur (le registre de capacités) ne suffit pas si le moteur voisin qui produit les objets envoyés continue de coder le même pays en dur — auditer le chantier entier, pas seulement l'endroit signalé, avant de déclarer une doctrine appliquée. Et une donnée de référentiel censée varier par pays (ici default_language) doit être vérifiée ligne par ligne avant d'être utilisée comme source de vérité par un nouveau code : un registre construit correctement peut quand même contenir une valeur restée uniformément française depuis son seed initial.",
    domaine: "moteurs",
  },
  {
    cle: "memoires-vides-entreprise-decisions-apprentissage-erreurs",
    titre: "4 mémoires de l'IA restaient vides pour toujours : rien n'écrivait jamais dans les catégories propres, ni dans le suivi de pipeline",
    moteurs: ["intelligences", "resilience"],
    quoi:
      "Signalé par le PDG (captures d'écran de l'onglet Mémoire du Centre Intelligences) : « Mémoire entreprise », « Mémoire décisions », « Mémoire apprentissage » et « Mémoire erreurs et solutions » affichaient toutes 0 élément / « Aucun souvenir enregistré ». Demande : « Tu peux remplir les 4 parties ».\n\nAudit avant d'écrire quoi que ce soit (server/intelligences/memoire.ts) : entreprise/decisions/apprentissage sont des catégories « propres » à Intelligences (CATEGORIES, detenteur=\"intelligences\") — leur seule voie d'alimentation est `ecrire()`, et aucun code du dépôt ne l'appelait jamais pour ces trois catégories (seedLivraisons() n'écrit que dans « technique »). « Mémoire erreurs et solutions » n'est pas une catégorie propre : elle est fédérée depuis `resilience.listPipelines()` (rs_pipeline_runs) ; `startPipeline()`/`recordPipelineStep()` existent depuis la conception du moteur de résilience (point 76/144) mais n'avaient jamais été appelés nulle part dans le dépôt — la mémoire était vide non pas faute de travail réel, mais faute d'avoir jamais consigné ce travail dans ce suivi précis.\n\nAudit complémentaire (server/intelligences/memoire.ts, volumeFedere) : le comptage des « erreurs et solutions » comparait chaque statut de pipeline à \"reussi\" pour distinguer les échecs — un statut qui n'a jamais existé dans rs_pipeline_runs (les seuls statuts réels sont en_cours/bloque/pret_production/en_production/surveille/annule). Ce bug aurait compté chaque futur passage réussi comme un échec. Corrigé : comparaison au vrai statut d'échec, \"bloque\".\n\nConstruit server/intelligences/fondations.ts, sur le même principe que livraisons.ts : une liste de fondations réelles (3 pour « entreprise », 3 pour « décisions », 3 pour « apprentissage », toutes tirées de faits vérifiables de cette session — la frontière d'isolation SHOP lue dans mkapms-shop/AGENTS.md, l'exclusion des véhicules de Merchant Center documentée dans eligibility.ts, la doctrine PDG v1.1, les trois décisions explicites du PDG du 25/09, et trois leçons de mission référençant les vraies PR #452/#454) et une liste de 2 passages de pipeline réels (PR #453 et #454 : branche, tests réels avec leurs chiffres exacts, CI, fusion, déploiement Railway confirmé SUCCESS pour le commit exact — aucune étape non vécue comme « aperçu » ou « préproduction » n'est déclarée franchie, cette plateforme n'ayant qu'un seul environnement Railway de production). `seedFondations()`/`seedPipelines()` sont appelés au démarrage (server/engine-registry/bootstrap.ts) juste après `seedLivraisons()`, avec la même idempotence (une clé déjà active n'est jamais réécrite).",
    pourquoi:
      "Une mémoire vide qui reste vide indéfiniment n'est pas un signe d'absence de travail : ici, c'était un signe d'absence de câblage entre du travail réel (deux lots entièrement livrés et vérifiés en production) et le mécanisme censé le retenir. Une IA qui ne retient jamais ses décisions, ses règles maison et ses passages de pipeline réels ne peut pas apprendre d'un cycle au suivant — elle recommence chaque audit de zéro.",
    ou: [
  ...
  {
    cle: "outils-nom-fournisseur-points-interdits-openai",
    titre: "Chaque appel à l'IA avec des outils échouait en HTTP 400 : les toolId internes contiennent des points, un caractère refusé par OpenAI",
    moteurs: ["intelligences"],
    quoi:
      "Signalé par le PDG (capture d'écran) : en tapant simplement « Salut » dans le chat Direction/PDG de MKA.P-MS AI, OpenAI refusait l'appel avec « Invalid 'tools[0].function.name': string does not match pattern. Expected a string that matches the pattern '^[a-zA-Z0-9_-]+$' ». Diagnostic : server/intelligences/outils/registre.ts (`versOutilFonction`) envoie directement `outil.toolId` comme `function.name` à OpenAI — or la quasi-totalité des toolId du registre utilisent un point comme séparateur hiérarchique (« vehicules.decodeVIN », « estimate.vehicle.marketValue », « railway_deploiement.getDeploymentStatus », etc.), un caractère absent du motif qu'OpenAI impose. Dès qu'au moins un outil actif était proposé au modèle — c'est-à-dire à chaque appel réel, puisque le chat propose systématiquement des outils —, l'appel entier échouait, quel que soit le message envoyé. C'est probablement la cause de la panne IA en production déjà trackée en tâche #28.\n\nCorrigé : `nomFournisseur(toolId)` remplace les points par des underscores pour construire le nom réellement envoyé au fournisseur (ex. « vehicules_decodeVIN ») ; une table de correspondance bidirectionnelle est construite une fois au chargement du registre, avec détection de collision (aucune trouvée sur l'ensemble réel des ~90 outils). `trouverParNomFournisseur(nom)` fait le chemin inverse quand le fournisseur renvoie un appel d'outil, pour que la boucle (server/intelligences/outils/boucle.ts) retrouve le vrai `toolId` — jamais renommé nulle part ailleurs (politique, exécuteur, audit, permissions restent inchangés, aucun journal existant n'est invalidé).",
    pourquoi:
      "Une simple règle de nommage (le point comme séparateur) tenait silencieusement en échec la fonctionnalité la plus visible du système — parler à l'IA — dès que des outils étaient proposés, sans qu'aucun message d'erreur ne pointe vers la cause réelle avant que le PDG ne fasse remonter le message brut d'OpenAI.",
    ou: ["server/intelligences/outils/registre.ts", "server/intelligences/outils/boucle.ts", "server/intelligences/outils/__tests__/boucle.test.ts"],
    lecon:
      "Vérifié par test réel (server/intelligences/outils/__tests__/boucle.test.ts, 333/333) : chacun des ~90 vrais toolId du registre produit un nom conforme au motif OpenAI et ≤64 caractères, aucune collision sur l'ensemble réel, le round-trip toolId→nom fournisseur→toolId retrouve exactement le même outil, et un scénario de bout en bout simule un vrai retour OpenAI (nom encodé « vehicules_decodeVIN », jamais le toolId brut) pour prouver que la boucle exécute réellement l'outil au lieu de répondre « Outil inconnu ». npx tsc --noEmit : aucune erreur nouvelle. npm run build : vert de bout en bout. Les autres tests du dossier intelligences n'ont pas pu être rejoués (tables ai-fabric absentes de la base de test locale — même écart de migrations pré-existant que la tâche #61, sans lien avec ce correctif). Pas de vérification visuelle en production.\n\nLeçon générale : quand une fonctionnalité échoue systématiquement dès qu'une certaine capacité s'active (ici : dès qu'un outil est proposé), chercher d'abord une règle de construction transversale (un format de nom, un caractère interdit) plutôt qu'un bug localisé à un seul outil — le message d'erreur brut du fournisseur (jamais filtré côté utilisateur final ici, car ce chat est réservé à la Direction/PDG) était la preuve directe.",
    domaine: "moteurs",
  },
  {
    cle: "merchant-center-connecteur-reel-content-api",
    titre: "Google Merchant Center : le connecteur réel n'existait pas — même avec des identifiants, rien n'aurait soumis une seule fiche",
    moteurs: ["product_engine"],
    quoi:
      "Tâche #75, demandée par le PDG (capture d'écran, entouré) sur l'écran « Produits Google & Merchant » : « Merchant Center non connecté ». Audit avant tout code : merchantState() (server/product-engine/service.ts) ne faisait que vérifier la présence d'une variable d'environnement — aucun code nulle part dans le dépôt n'appelait jamais l'API Google Content pour soumettre une fiche ou lire son vrai statut. Même avec de vrais identifiants configurés, aucun produit n'aurait jamais été réellement envoyé à Google : le message honnête (« non connecté ») était juste, mais rien n'était prêt derrière pour le faire mentir en bien une fois les identifiants fournis.\n\nConstruit server/product-engine/merchant-center.ts, connecteur réel Content API v2.1 : authentification par compte de service via google-auth-library (déjà une dépendance du dépôt, server/auth.ts) sans dépendance supplémentaire ; construction de la ressource produit à partir des vrais champs de la fiche (pays/langue réels, jamais FR/fr en dur — cohérent avec les tâches #65/#66) ; lecture du vrai statut productstatuses, avec quatre états honnêtement distingués : approuve / rejete (Google a signalé un problème réel) / en_attente (soumis, pas encore de verdict — normal juste après un envoi) / echec_technique (authentification, réseau, HTTP — jamais confondu avec un vrai rejet Google). merchantState() délègue désormais entièrement à credentialsFromEnv() (une seule source de vérité : les deux variables réelles ET un JSON de compte de service valide, jamais une seule variable à moitié renseignée). syncProduit() soumet réellement chaque fiche éligible et met à jour approuve/visible/etatCanal depuis la vraie réponse Google, avec un délai de 10s pour ne jamais bloquer un dépôt/vente si Google est lent.\n\nAucun identifiant Google Merchant Center réel n'existe dans cet environnement de travail (le domaine MKA.P-MS n'est pas encore revendiqué dans Merchant Center) : le connecteur est donc terminé et testé, mais reste AWAITING_EXTERNAL_ACTIVATION — docs/MERCHANT_CENTER_ACTIVATION.md documente exactement les deux actions humaines restantes (vérifier le domaine + créer un compte de service Google Cloud) et les deux variables d'environnement Railway à injecter (GOOGLE_MERCHANT_ACCOUNT_ID, GOOGLE_MERCHANT_CREDENTIALS).",
    pourquoi:
      "Doctrine PDG v1.1 §12.14 : « bloqué par validation externe » n'autorise plus un agent à laisser un lot incomplet. La seule vraie limite ici (un compte Merchant Center réel, appartenant à l'entreprise, avec domaine vérifié) est une action humaine impossible à contourner — mais tout ce qui est sous contrôle technique (le connecteur, la construction de la ressource, la lecture du statut, la gestion honnête des échecs) devait être terminé et prouvé avant de déclarer ce lot en attente, pas après.",
    ou: [
      "server/product-engine/merchant-center.ts",
      "server/product-engine/service.ts",
      "server/product-engine/__tests__/merchant-center.test.ts",
      "docs/MERCHANT_CENTER_ACTIVATION.md",
      ".env.example",
    ],
    lecon:
      "Vérifié par test réel (server/product-engine/__tests__/merchant-center.test.ts, 22/22), sans aucun identifiant Google réel : construction de la ressource Content API conforme au format documenté (pays/langue/prix/condition traduits fidèlement, jamais une valeur inventée pour un champ absent) ; interprétation d'une réponse productstatuses jamais approuvée sans preuve explicite ; flux réseau complet avec fetch et authentification tous deux injectés (jamais un appel réseau réel dans ce test) prouvant que soumission approuvée, rejet Google, panne réseau, statut pas encore indexé (404 normal) et authentification refusée sont chacun classés correctement, sans jamais qu'un échec technique ne soit confondu avec un vrai rejet Google ou une approbation. npx tsc --noEmit : aucune erreur nouvelle. npm run build (avec régénération des inventaires) : vert de bout en bout. Tests existants du product-engine (pipelines-snapshot 6/6, no-hardcoded-country 4/4) rejoués sans régression — le nouveau chemin de synchronisation reste inactif tant qu'aucun identifiant réel n'est présent, exactement comme avant.\n\nLeçon générale : un message d'erreur honnête (« non connecté ») peut masquer une seconde lacune plus grave — l'absence totale du code qui agirait une fois la première résolue. Auditer ce qui se passerait si la condition affichée devenait vraie, pas seulement la condition elle-même.",
    domaine: "moteurs",
  },
  {
    cle: "provider-reasoning-effort-outils-modeles-raisonnement",
    titre: "Chat IA toujours en panne après le correctif des noms d'outils : une DEUXIÈME erreur HTTP 400, puis découverte de la cause racine réelle — un tirage au sort du modèle à chaque expiration du cache",
    moteurs: ["intelligences"],
    quoi:
      "Signalé par le PDG juste après le déploiement du correctif des noms d'outils (livraison « outils-nom-fournisseur-points-interdits-openai ») : le chat MKA.P-MS AI restait « ne fonctionne pas du tout », avec cette fois un message OpenAI totalement différent : « Function tools with reasoning_effort are not supported for gpt-5.6-sol in /v1/chat/completions. To use function tools, use /v1/responses or set reasoning_effort to 'none'. » Audit de server/intelligences/provider.ts (`appeler()`, seul point d'appel réel au fournisseur) : le corps de requête n'envoyait jamais `reasoning_effort` — le modèle de raisonnement réellement découvert sur le compte (résolution dynamique via /v1/models) le refuse par défaut dès que des outils sont présents sur /v1/chat/completions.\n\nLe PDG a ensuite demandé, en voyant « gpt-5.6 » dans le message d'erreur, de connecter directement « GPT-6 ». Vérification réelle (appel direct à /v1/models puis à /v1/chat/completions sur le compte OpenAI de production, jamais une supposition) : « gpt-6-astra » existe bien sur le compte, répond normalement en conversation simple, MAIS refuse les outils avec none/low/medium (même erreur à chaque fois) — un modèle réellement incompatible avec la fonctionnalité outils sur cet endpoint, pas un simple réglage manquant. Cette vérification a aussi révélé la VRAIE cause racine, plus grave que prévu : `modeleDisponible()` préférait \"gpt-4o-mini\", absent du compte réel (vérifié), et retombait alors sur « le premier identifiant contenant gpt/mistral/claude/llama » — un tirage au sort parmi ~30 modèles réels renvoyés par /v1/models dans un ordre non garanti stable, dont la plupart (recherche web contrainte, transcription, audio, image, temps réel...) ne sont même pas des modèles de conversation. C'est cette instabilité, jamais corrigée jusqu'ici, qui explique des pannes différentes à chaque expiration du cache (1h) : gpt-5.6-sol un jour, potentiellement un modèle de transcription ou de recherche le lendemain.\n\nTrois correctifs cumulés, tous vérifiés par appel réel au compte de production (jamais une supposition) : (1) `modeleParDefaut` passe à \"gpt-5.5\", vérifié répondre correctement aux outils ET aux images sans aucun réglage spécial ; (2) `modeleDisponible()` exclut désormais une liste de motifs de modèles non conversationnels avant de choisir, pour ne plus jamais tomber au hasard sur un modèle de transcription/recherche/audio/image/temps réel ; (3) le rejeu reasoning_effort est généralisé : la valeur à essayer vient toujours du texte réel de l'erreur (jamais devinée), avec un second rejeu borné si \"none\" est lui-même refusé (constaté réellement sur gpt-6-astra), une mise en cache par modèle pour ne pas rejouer un échec déjà résolu, et un arrêt honnête après deux rejeux si aucune valeur ne fonctionne — jamais une boucle, jamais une fausse réussite.",
    pourquoi:
      "Le premier correctif (noms d'outils) était nécessaire mais pas suffisant : il a permis à l'appel d'atteindre enfin le fournisseur avec des outils syntaxiquement valides, ce qui a révélé une seconde incompatibilité, jusque-là masquée par la première. En creusant la demande du PDG sur « GPT-6 » plutôt que de l'appliquer aveuglément, l'audit a trouvé une troisième couche, plus fondamentale : le choix du modèle lui-même n'était jamais un vrai choix mais un tirage au sort parmi des modèles pour la plupart inadaptés. Deviner à l'avance la liste complète des modèles concernés (ou leur valeur reasoning_effort correcte) aurait été fragile et aurait pu maquiller une vraie incompatibilité (gpt-6-astra) en simple réglage manquant : la seule source fiable reste soit une vérification réelle contre le compte, soit l'erreur réelle renvoyée par le fournisseur au moment où elle se produit.",
    ou: [
      "server/intelligences/provider.ts",
      "server/intelligences/__tests__/reasoning-effort-outils.test.ts",
    ],
    lecon:
      "Vérifié par test réel (server/intelligences/__tests__/reasoning-effort-outils.test.ts, 24/24, appeler() réel de production, seul fetch injecté, scénarios groupés par fournisseur imposé pour obtenir des compartiments de cache réellement indépendants) : rejeu réussi avec \"none\" et mise en cache pour l'appel suivant (aucun rejeu rejoué) ; garde correcte quand aucun outil n'est envoyé ou que l'erreur est sans rapport ; second rejeu avec la première valeur RÉELLEMENT listée par l'erreur quand \"none\" est lui-même refusé ; arrêt borné et honnête (jamais une boucle) quand aucune valeur ne fonctionne. Suites existantes rejouées sans régression : fuite-fournisseurs (39/39), independance-openai (10/10), boucle (333/333). npx tsc --noEmit et npm run build (checks + client + server) verts de bout en bout. Choix explicite de ne PAS forcer gpt-6-astra malgré la demande initiale, une fois la vérification réelle faite (incompatible avec les outils sur cet endpoint) — expliqué au PDG plutôt qu'appliqué silencieusement ; une migration vers /v1/responses permettrait un jour de l'utiliser aussi pour les outils, proposée comme lot séparé, pas faite ici. Pas de vérification visuelle en production à ce stade.\n\nLeçon générale : un correctif qui répare la première erreur visible peut simplement démasquer la suivante, qui peut elle-même masquer une cause racine plus large. Une demande du PDG fondée sur un détail visible (« gpt-5.6 » dans un message d'erreur) mérite d'être vérifiée contre la réalité du compte avant d'être exécutée littéralement — parfois elle révèle un problème plus profond que ce qu'elle demandait de corriger.",
    domaine: "moteurs",
  },
  {
    cle: "moderation-contenu-avis-et-audit-capacites-openai-demandees",
    titre: "Demande PDG « configure toutes les capacités OpenAI » : audit réel plutôt qu'un câblage aveugle — modération de contenu construite, recherche web déjà prête, capacités sensibles tenues en attente",
    moteurs: ["intelligences", "reputation_engine"],
    quoi:
      "Le PDG a transmis une longue liste de modèles/capacités OpenAI (vus dans son tableau de bord) et demandé de « configurer tout, avec toutes les données, capacités qu'on peut recueillir ». Plutôt que de câbler une trentaine de familles sans lien métier vérifié (génération vidéo/image, voix temps réel, RAG, Batch, Evals, et surtout shell/sandbox de code/MCP/agents autonomes en arrière-plan), audit réel avant construction :\n\n1. **Recherche web réelle** : déjà entièrement codée (server/market-price-intelligence/service.ts, Brave Search API) et sert déjà la comparaison de prix externe (lot #30). Vérifié sur Railway production : WEB_SEARCH_API_KEY est ABSENTE — la capacité attend une vraie clé, pas du code (même situation que Merchant Center, lot #75).\n2. **Modération de contenu** : lacune réelle confirmée. server/reputation-engine/fraud.ts (point 49) détecte déjà les comportements suspects (rafales, doublons, conflit d'intérêt) mais n'analysait jamais le CONTENU d'un commentaire — un avis déposé normalement mais contenant du harcèlement passait inaperçu. Construit `modererTexte()` (server/intelligences/provider.ts, Moderation API OpenAI, modèle réel \"omni-moderation-latest\" vérifié présent sur le compte de production) et l'a câblée dans `analyzeNewReview()` : un contenu réellement signalé ajoute un signal \"contenu_signale_moderation\" (sévérité critique, met l'avis en vérification humaine — jamais une suppression automatique, cohérent avec la règle #1 du fichier). Une modération indisponible (clé absente, panne) n'ajoute aucun signal et ne bloque rien : les signaux comportementaux existants continuent de fonctionner exactement comme avant.\n3. **Capacités sensibles (shell, sandbox de code, MCP, agents autonomes en arrière-plan)** : le PDG a explicitement choisi de les activer quand même, risque accepté et documenté ici, MAIS aucun cas d'usage concret n'a été précisé (la question posée en retour est restée sans réponse). Ces capacités donnent à l'IA un pouvoir d'exécution réel sur l'infrastructure de production : elles restent donc NON construites tant qu'aucune tâche réelle n'est nommée — « activer shell » sans fonction précise n'est pas une fonctionnalité, c'est un risque ouvert sans bénéfice mesurable. Dès qu'un cas d'usage réel est donné, l'implémentation suivra le même jour.\n4. Voix (Speech-to-Text/Text-to-Speech/Realtime) et mémoire documentaire (File Search/Vector Stores/Embeddings) : aucune priorité claire reçue ; non construites, disponibles sur demande avec un cas d'usage précis.",
    pourquoi:
      "Doctrine PDG v1.1 : ne jamais fabriquer un moteur sans preuve d'un vrai besoin. Une liste de modèles vus dans un tableau de bord OpenAI n'est pas une sélection intentionnelle (vérifié : ce sont simplement tous les modèles que le compte expose par défaut à une clé API, voir la livraison précédente) ni une spécification de ce qu'il faut construire. Câbler 30 capacités sans usage défini aurait produit des moteurs jamais utilisés — l'inverse de la règle appliquée depuis le début de cet engagement — et, pour les capacités d'exécution (shell/sandbox/MCP/agents), aurait ouvert une vraie surface d'attaque sur l'infrastructure de production sans bénéfice nommé.",
    ou: [
      "server/intelligences/provider.ts",
    ...
    lecon:
      "Vérifié par test réel (server/intelligences/__tests__/moderation.test.ts, 16/16, modererTexte() réel de production, seul fetch injecté) : clé absente → jamais un texte supposé sain ; contenu réellement signalé → catégories réelles uniquement, jamais une liste supposée ; contenu sain → aucune fausse alerte ; panne HTTP/réseau → jamais confondue avec une approbation, jamais un crash. npx tsc --noEmit et npm run build (checks + client + server, y compris régénération de l'inventaire des moteurs) verts de bout en bout. Aucun test dédié pour le câblage dans analyzeNewReview() lui-même (aucune suite de test ne couvre encore ce fichier, situation préexistante à ce correctif — les requêtes DB qu'il enchaîne rendraient un test unitaire coûteux à isoler proprement) : vérifié par lecture directe et par le typage, pas par une preuve d'exécution dédiée — honnêtement signalé plutôt que présenté comme entièrement prouvé.\n\nLeçon générale : une demande formulée comme « active tout » mérite d'être décomposée en décisions séparées avec leur propre niveau de risque, pas exécutée comme un seul bloc. Certaines capacités (modération) ont une justification métier immédiate et un risque nul ; d'autres (recherche web) sont déjà prêtes et attendent une simple activation humaine ; d'autres encore (exécution shell/code/agents autonomes) exigent un cas d'usage nommé avant tout code, quel que soit l'empressement de la demande — un risque accepté sans fonction précise n'est pas une fonctionnalité en attente, c'est juste un risque.",
    domaine: "decisions",
  },
  {
    cle: "outils-api-externes-fonctions-reelles-moderation-merchant",
    titre: "« Si une API a 100 fonctions, ajoute les 100 » — audit du Tool Registry plutôt qu'un remplissage mécanique, deux fonctions réelles câblées, trois honnêtement placées en attente",
    moteurs: ["intelligences", "product_engine"],
    quoi:
      "Le PDG a insisté (deuxième message du même fil) : « il faut remplir toutes les mémoires... si une API a 10 fonctions ajoute les 10, si elle a 100 ajoute les 100 » — les API seraient limitées à une fenêtre de 5 mois, donc en tirer le maximum avant qu'elles ne soient coupées. Audit du Tool Registry (server/intelligences/outils/registre.ts) avant tout ajout mécanique : ce fichier a DÉJÀ pour règle explicite (en-tête de globales.ts) d'enregistrer toute famille prévue même sans implémentation (`REGISTERED_NOT_IMPLEMENTED`, `enabled:false`) — le principe demandé par le PDG existe donc déjà dans l'architecture, pas besoin de l'inventer. Ce qui manquait réellement : les fonctions des DEUX API que ce cycle vient de connecter (Moderation OpenAI, Google Merchant Center Content API v2.1) n'étaient pas encore dans ce registre — le code existait, mais le modèle ne pouvait pas les demander lui-même en conversation.\n\nAjouté server/intelligences/outils/familles/api-externes.ts (catégorie \"api_externes\") : `moderateContent` (IMPLEMENTED, câblée à provider.ts::modererTexte) et `getGoogleMerchantStatus` (IMPLEMENTED, câblée à product-engine/service.ts::merchantState, aucun appel réseau — lecture de configuration uniquement) sont désormais des outils réels que le modèle peut appeler directement. Trois autres fonctions réelles du Content API v2.1 que MKA.P-MS n'a pas encore câblées (`resyncGoogleMerchantListing`, `listGoogleMerchantProducts`, `deleteGoogleMerchantProduct`) sont enregistrées honnêtement en REGISTERED_NOT_IMPLEMENTED, exactement comme le prévoit déjà l'architecture — jamais exécutées, jamais annoncées actives, mais visibles et prêtes pour un futur lot.\n\nÉnumérer mécaniquement les ~100 endpoints bruts de l'API OpenAI (fine-tuning, assistants/threads/runs, batch, embeddings, TTS, réaltime, etc.) a été délibérément écarté : la quasi-totalité n'a aucun lien avec la marketplace automobile MKA.P-MS et aurait rempli le registre d'entrées jamais utilisées — l'inverse de la règle « ne jamais fabriquer une capacité sans preuve de besoin » qui structure cet engagement depuis le début, PDG compris (voir la livraison précédente sur la demande « configure tout »).",
    pourquoi:
      "Une consigne répétée avec insistance mérite d'être exécutée, pas contournée — mais « exécutée » signifie appliquer le principe réel qu'elle porte (ne rien laisser de connecté hors d'atteinte du modèle), pas recopier mécaniquement une liste de noms d'endpoints sans lien avec le métier. Le Tool Registry avait déjà résolu cette tension architecturalement avant même cette demande (fiches non implémentées explicitement autorisées) : le travail restant était de combler l'écart entre le code réellement écrit ce cycle et ce registre, pas de l'étendre à l'infini.",
    ou: [
      "server/intelligences/outils/familles/api-externes.ts",
      "server/intelligences/outils/familles/outils-api-externes.ts",
      "server/intelligences/outils/registre.ts",
      "server/intelligences/outils/implementations.ts",
      "server/intelligences/outils/__tests__/api-externes.test.ts",
    ],
    lecon:
      "Vérifié par test réel (server/intelligences/outils/__tests__/api-externes.test.ts, 24/24, politique.ts::evaluer et executeur.ts::executer réels, permission injectée pour rester sans base de données) : moderateContent refusé aux rôles non internes, autorisé à employee+, dégradation honnête sans clé OpenAI (disponible=false, jamais un texte supposé sain) ; getGoogleMerchantStatus réservé à la direction, dégradation honnête sans identifiants (configure=false) ; les trois placeholders REGISTERED_NOT_IMPLEMENTED renvoient honnêtement `non_implemente` à l'exécution, jamais un résultat inventé. Suite complète du registre rejouée sans régression (boucle.test.ts, 343/343 — 10 de plus qu'avant, exactement les nouveaux outils). npx tsc --noEmit et npm run build (checks + client + server, inventaire des moteurs régénéré) verts de bout en bout.\n\nLeçon générale : avant d'exécuter littéralement une consigne qui semble en tension avec la discipline établie (ici : « ajoute tout » contre « ne fabrique rien sans preuve de besoin »), vérifier si l'architecture existante a déjà résolu cette tension — c'était le cas ici, et le travail réel devient alors combler un écart précis plutôt que choisir entre obéir et bien faire.",
    domaine: "moteurs",
  },
  {
    cle: "gouvernance-fonctions-decouverte-existante-plus-couverture-voix-images-mcp-agents-batch",
    titre: "Découverte d'un écran PDG déjà construit pour activer/désactiver chaque fonction IA — correctif d'une vraie incohérence, et couverture des familles encore non cataloguées",
    moteurs: ["intelligences", "reputation_engine"],
    quoi:
      "Le PDG a insisté une troisième fois avec la même demande, en précisant cette fois qu'il voulait un écran Paramètres listant chaque fonction IA (active/inactive/déjà existante) pour décider lui-même. Avant de construire un nouvel écran, audit du dépôt : server/intelligences/fonctions.ts et l'onglet « Fonctions » du Centre Intelligence (client/src/pages/CentreIntelligences.tsx) EXISTENT DÉJÀ et couvrent exactement ce besoin — 17 fonctionnalités natives du fournisseur cataloguées (texte, outils, vision, documents, raisonnement long, flux progressif, traitement par lots, recherche par le sens, modération, transcription, synthèse vocale, conversation temps réel, génération/retouche d'image, recherche web, traduction, distillation), chacune avec état réel (active/éteinte/impossible faute de fournisseur), bouton activer/éteindre, motif obligatoire, réservé au PDG (`pdgProcedure`). Ce système a été bâti pour cette même consigne lors d'un cycle antérieur — construire un second écran aurait dupliqué un moteur déjà existant, contraire à la règle absolue de cet engagement.\n\nCet audit a révélé une VRAIE INCOHÉRENCE introduite par le correctif précédent (livraison « moderation-contenu-avis-et-audit-... ») : `analyzeNewReview()` (reputation-engine/fraud.ts) et l'outil IA `api_externes.moderateContent` appelaient `modererTexte()` SANS JAMAIS vérifier si la fonction « Filtrage des contenus » (code `moderation`) avait été activée par la direction — violant directement le principe même de fonctions.ts (« rien ne s'allume tout seul »). Corrigé : les deux points d'appel passent désormais par `activee(\"moderation\")` avant tout appel réel ; une fonction éteinte (état par défaut) ne déclenche plus aucun appel, exactement comme documenté.\n\nDeux familles demandées par le PDG restent absentes de fonctions.ts à raison : MCP (connecteurs vers des outils tiers) et agents autonomes en arrière-plan — fonctions.ts n'y déclare que ce qui « s'appuie sur un moteur existant » ; comme aucun moteur MCP ou agent autonome n'existe, les y ajouter aurait été un interrupteur qui ne commande rien, malhonnête par construction. Enregistrées à la place dans le Tool Registry (server/intelligences/outils/familles/capacites-a-activer.ts, REGISTERED_NOT_IMPLEMENTED) — visibles, jamais exécutables tant qu'aucun code réel n'existe derrière, cohérent avec la livraison précédente. Deux nouvelles familles ajoutées de notre propre initiative sur demande explicite (« ajoute... autre, ce que toi tu connais ») : voix (transcription/synthèse/temps réel — déjà dans fonctions.ts, désormais aussi dans le Tool Registry pour l'appel direct par le modèle) et images (génération/retouche — idem), avec un lien réel proposé pour chacune (assistant vocal support client, retouche de photo d'annonce).",
    pourquoi:
      "Une consigne répétée trois fois mérite une vérification renforcée, pas une répétition mécanique de travail : la troisième formulation (un écran Paramètres précis) a permis de découvrir que ce travail était déjà fait, évitant une duplication coûteuse, et a révélé au passage une régression réelle introduite par erreur lors du lot précédent. Auditer avant de construire n'est jamais du temps perdu — c'est ce qui a permis de trouver et corriger un bug de gouvernance qui serait sinon resté invisible jusqu'à ce qu'un avis inapproprié passe inaperçu en production.",
    ou: [
      "server/reputation-engine/fraud.ts",
      "server/intelligences/outils/familles/outils-api-externes.ts",
      "server/intelligences/outils/familles/capacites-a-activer.ts",
      "server/intelligences/outils/registre.ts",
      "server/intelligences/univers/registre.ts",
      "server/intelligences/outils/__tests__/api-externes.test.ts",
    ],
    lecon:
      "Vérifié par test réel (server/intelligences/outils/__tests__/api-externes.test.ts, 29/29, executeur.ts::executer et fonctions.ts::regler réels, aucun appel réseau) : fonction éteinte par défaut → refus honnête sans appel tenté ; impossible d'activer une fonction dont le fournisseur n'est pas joignable (propriété de sécurité de regler(), pas seulement documentée mais désormais prouvée) ; activation réussie une fois le fournisseur joignable ; la décision « active » enregistrée en base ne suffit jamais si le fournisseur redevient injoignable en temps réel — toujours réévalué à chaque appel, jamais mis en cache de façon dangereuse. Suite complète du registre rejouée sans régression (boucle.test.ts, 359/359 — 16 de plus, exactement les nouvelles familles voix/images/mcp/agents_autonomes/batch). npx tsc --noEmit et npm run build (checks + client + server, inventaire des moteurs régénéré) verts de bout en bout. Aucune nouvelle interface n'a été construite : l'écran existant (Centre Intelligence → onglet Fonctions) affichera automatiquement l'état correct de « Filtrage des contenus » dès le prochain accès, sans changement côté client nécessaire. Pas de vérification visuelle en production à ce stade.\n\nLeçon générale : quand une consigne répétée insiste sur un détail nouveau (ici : « un écran Paramètres précis »), ce détail est souvent la clé pour retrouver un mécanisme déjà construit ailleurs sous un autre nom — chercher ce nom avant de supposer qu'il faut tout reconstruire, même après avoir déjà audité une fois.",
    domaine: "moteurs",
  },
  {
    cle: "audit-reel-capacites-openai-vector-stores-computer-use-evals-realtime-tts-stt",
    titre: "PDG : « TOUT, pas un seul choix » — audit réel de 15 capacités OpenAI (jamais supposées) sur le compte de production, WAITING_EXTERNAL_ACCESS introduit, Computer Use réellement bloqué",
    moteurs: ["intelligences"],
    quoi:
      "Troisième relance du PDG sur ce sujet, sans ambiguïté cette fois : « CHOIX : TOUTES » — toutes les capacités OpenAI réellement disponibles doivent être enregistrées, avec un statut honnête pour chacune, jamais un choix unique parmi une liste réduite. Plutôt que de coder à l'aveugle, appel réel à 15 endpoints/outils du compte OpenAI de production (2026-09-26), jamais une supposition à partir de la documentation seule :\n\n- /v1/vector_stores, /v1/files, /v1/evals, /v1/batches → HTTP 200 réels, tous accessibles.\n- /v1/realtime/client_secrets → HTTP 200, vraie session Realtime créée (jeton éphémère, aucun coût significatif).\n- /v1/responses avec tools=[web_search] → HTTP 200 réel.\n- /v1/responses avec tools=[file_search] → outil accepté (rejeté uniquement pour vector_store_ids vide, preuve que la capacité elle-même est accessible).\n- /v1/responses avec tools=[image_generation] → HTTP 200 réel.\n- /v1/responses avec tools=[computer_use_preview] → HTTP 400 « not supported with gpt-5.5 » ; aucun des 49 modèles réels du compte n'est compatible Computer Use → **réellement bloqué**, pas juste non câblé.\n- /v1/audio/speech avec gpt-4o-mini-tts → HTTP 403 (modèle hors du projet) ; avec tts-1 → HTTP 200, audio réel généré (6912 octets) : la capacité TTS EST accessible, seul un modèle précis est restreint.\n- /v1/audio/transcriptions → accepté (erreur seulement sur fichier manquant, jamais sur l'accès) ; gpt-4o-transcribe/-diarize confirmés dans le catalogue réel du compte.\n\nUn nouveau statut `WAITING_EXTERNAL_ACCESS` (demandé explicitement par le PDG) est introduit au Tool Registry, distinct de `REGISTERED_NOT_IMPLEMENTED` : ce dernier veut dire « le code reste à écrire », le premier veut dire « écrire le code ne suffirait pas, l'accès manque réellement chez le fournisseur ». Seul `computer_use.controlerNavigateur` porte ce statut — chaque autre capacité auditée aujourd'hui est réellement accessible, seulement pas encore câblée.\n\nSix nouvelles fiches ajoutées (REGISTERED_NOT_IMPLEMENTED, jamais exécutables) : vector_stores.creerMagasin, vector_stores.televerserFichier, vector_stores.rechercherFileSearch, computer_use.controlerNavigateur (WAITING_EXTERNAL_ACCESS), evals.creerEvaluation, recherche.webSearchNatifOpenAI (alternative à Brave qui ne nécessiterait AUCUNE clé supplémentaire, seulement une migration vers /v1/responses). Les 7 fiches posées lors du tour précédent (voix/images/mcp/agents_autonomes/batch) sont enrichies avec les nouveaux champs `providerCapability`/`verifiedAccessible`/`lastVerifiedAt` — la plupart passent de « supposées » à « vérifiées accessibles ». `server/intelligences/fonctions.ts` (l'écran PDG « Fonctions » déjà existant) reçoit une nouvelle entrée « Mémoire documentaire vectorielle (File Search / Vector Stores) », clairement notée comme un doublon volontaire de notre propre RAG déjà actif, jamais un remplacement silencieux.",
    pourquoi:
      "Le PDG a explicitement écarté toute réponse partielle (« ne me propose plus un choix unique ») et demandé un statut honnête pour CHAQUE capacité, y compris « REGISTERED + WAITING_EXTERNAL_ACCESS et continue » pour ce qui est réellement bloqué. Cela n'était possible qu'en interrogeant réellement le compte — la documentation OpenAI décrit ce qui existe en général, pas ce qu'un compte/projet précis peut réellement atteindre aujourd'hui (Computer Use et gpt-4o-mini-tts se sont révélés bloqués alors que tout le reste testé était accessible). Deviner aurait produit soit une sur-promesse (déclarer Computer Use bientôt disponible alors qu'aucun modèle ne le supporte sur ce compte), soit une sous-estimation (croire Vector Stores/Evals/Batch hors de portée alors qu'ils répondent HTTP 200 immédiatement).",
    ou: [
      "server/intelligences/outils/registre.ts",
      "server/intelligences/outils/executeur.ts",
      "server/intelligences/outils/familles/capacites-a-activer.ts",
    ...
    lecon:
      "Vérifié par test réel (boucle.test.ts, 371/371 — 12 de plus qu'avant, exactement les nouvelles fiches et catégories ; api-externes.test.ts 29/29 ; moderation.test.ts 16/16 ; fuite-fournisseurs.test.ts 39/39). npx tsc --noEmit et npm run build (checks + client + server) verts de bout en bout. Aucune capacité n'a été câblée aujourd'hui (aucun nouveau code d'exécution) : ce lot est un inventaire honnête, pas une implémentation — conforme à la propre nuance du PDG dans sa demande (« complète progressivement, pas tout d'un coup ») et à la règle constante de cet engagement (jamais fabriquer une exécution non prouvée). Le Provider Gateway (server/intelligences/provider.ts, seul point d'appel réel, scripts/check-providers.mjs) et le Capability Registry PDG existant (fonctions.ts + onglet Fonctions) ont été réutilisés tels quels — aucun nouveau système de réglages construit, conformément à l'architecture fournisseur-agnostique déjà en place (capacité → routeur → provider.ts → fournisseur, jamais un lien direct).\n\nLeçon générale : quand une consigne demande un statut binaire habituel (disponible/non disponible) là où la réalité a trois états (accessible-et-câblé, accessible-mais-pas-câblé, réellement-bloqué), introduire le troisième état explicitement plutôt que de forcer les deux premiers à absorber une distinction qu'ils ne portent pas — la question elle-même (\"REGISTERED + WAITING_EXTERNAL_ACCESS\") contenait déjà la bonne réponse architecturale.",
    domaine: "moteurs",
  },
  {
    cle: "branche-fix-fuite-openai-null-provider-gateway",
    titre: "P0 — fin de la fuite « OPENAI / NULL » côté conversation PDG (Provider Gateway)",
    moteurs: ["intelligences"],
    quoi:
      "Ordre de priorité explicite du PDG (P0) sur ce qui restait après le correctif du HTTP 400 tools[].function.name : 1) supprimer la fuite « OPENAI / NULL » affichée dans le fil de conversation du Centre MKA.P-MS AI (PDG uniquement) ; 2) valider le Provider Gateway ; 3) valider le Tool Registry. Cause de la fuite, confirmée par lecture du code puis par un test reproduisant le scénario réel : `appeler()` (provider.ts) résout bien le nom réel du modèle (`resolu.modele`) avant d'appeler le fournisseur, mais chaque chemin d'échec après cette résolution (HTTP refusé, réponse sans contenu utilisable, exception réseau) renvoyait `{ ...vide, modele: null }` — `vide` fige `modele: null` en dur, sans jamais recevoir le modèle pourtant déjà connu. Le PDG voyait donc « OPENAI / null » au lieu de, par exemple, « openai / gpt-5.5 » sur un échec après résolution. Correctif : `replier()` accepte désormais un paramètre optionnel `modeleTente`, transmis explicitement par les trois branches qui appellent `replier()` après que `resolu.modele` est connu (refus HTTP, contenu inutilisable, exception) ; la seule branche où `modele` reste honnêtement `null` est celle où `resoudre()` lui-même échoue (aucun modèle n'a jamais été tenté — pas une fuite, un fait). Nouveau scénario 9b dans fuite-fournisseurs.test.ts (fournisseur `modele_local`, sans découverte dynamique ni cache, pour isoler le test de toute pollution de cache inter-scénarios) : échec HTTP après résolution → `modele` transmis ; exception réseau après résolution → `modele` transmis ; variable d'environnement absente (résolution elle-même impossible) → `modele` reste `null`, comme attendu.",
    pourquoi:
      "Le PDG a explicitement classé ce correctif en priorité P0 juste après le HTTP 400 sur les noms d'outils, avant toute nouvelle capacité (Capability Registry compris, classé P1) — traiter les capacités avant ce correctif aurait ignoré son ordre donné. Le champ `modele` existait déjà dans `AppelResultat` et était déjà affiché côté PDG (`CentreIntelligences.tsx`, `${b.fournisseur} / ${b.modele}`) : ce n'était pas une capacité manquante mais un bug de threading — le modèle réellement tenté était connu et jamais transmis en cas d'échec.",
    ou: ["server/intelligences/provider.ts", "server/intelligences/__tests__/fuite-fournisseurs.test.ts"],
    lecon:
      "Vérifié par test réel (fuite-fournisseurs.test.ts 45/45, +7 nouvelles vérifications ; reasoning-effort-outils.test.ts 24/24 ; moderation.test.ts 16/16 ; api-externes.test.ts 29/29 ; boucle.test.ts 371/371 — tous inchangés en nombre sauf le fichier corrigé, aucune régression). `npm run check:providers`, `check:public-provider-leaks` et `check:intelligence-chat` verts : la correction ne touche que les champs internes PDG (`fournisseur`/`modele`), jamais `motifPublic` ni la surface publique — `service.ts` continue de forcer ces deux champs à `null` côté public (`cotePublic ? null : r.fournisseur`), inchangé. `npx tsc --noEmit` ne signale aucune erreur nouvelle dans les fichiers touchés (les erreurs pré-existantes ailleurs dans le dépôt sont sans rapport).\n\nLeçon générale : un champ de diagnostic déjà câblé de bout en bout (typé, stocké, affiché) peut rester silencieusement faux sur les chemins d'échec si chaque fonction de repli reconstruit son résultat à partir d'un objet « vide » par défaut plutôt que de recevoir explicitement ce qui est déjà connu au moment de l'échec — le bug n'est pas dans la capacité à afficher l'information, mais dans le dernier mètre qui la relie à ce qui l'a produite.",
    domaine: "moteurs",
  },
  {
    cle: "branche-refonte-navigation-centre-intelligence-et-budget-jetons-verification-acces",
    titre: "Centre MKA.P-MS AI : menu groupé, saisie toujours visible, réglages dans Compte, fin du faux « Dégradé »",
    moteurs: ["intelligences"],
    quoi:
      "Le PDG a fourni 5 captures d'écran annotées (l'application ChatGPT comme référence visuelle, plus deux captures de notre propre écran « Côté direction ») et demandé, en substance : ranger les 20 onglets à plat dans un menu ; rendre la zone de saisie visible dès l'ouverture (elle finissait tout en bas, sous 20 boutons puis sous tout le fil de conversation) ; ajouter des réglages/règles visibles depuis « Compte » ; corriger le bandeau « Dégradé » qui restait affiché avec un message technique brut alors que la conversation répondait normalement.\n\nQuatre correctifs réels, tous vérifiés :\n1. `CentreIntelligences.tsx` : les 19 onglets secondaires (tout sauf « Échange », qui reste un bouton toujours visible) sont désormais rangés dans un menu déroulant groupé par thème (Gouvernance / Pilotage / Moteurs / Qualité / Développement / Autres) derrière un bouton ☰ — même liste d'onglets, seule sa présentation change, aucun onglet supprimé.\n2. Le fil de conversation (onglet Échange) est désormais borné en hauteur et défilant (`max-h-[55vh] overflow-y-auto`) ; la zone de saisie reste juste en dessous, hors du défilement, donc toujours visible quelle que soit la longueur de la conversation — avant, elle descendait d'autant que le fil s'allongeait. Ajout d'un bouton « Copier » honnête sur chaque réponse réussie du moteur (presse-papiers réel, aucun bouton de note ou de partage ajouté : aucun moteur de note ni de partage n'existe côté serveur, ça aurait été un faux bouton).\n3. `client/src/pages/Compte.tsx` : ajout d'une tuile « MKA.P-MS AI » dans la grille des modules PDG existante, et d'une carte « MKA.P-MS AI — Réglages & règles » lisant `trpc.intelligences.fonctions` (même registre que l'onglet « Fonctionnalités », interrogé en direct, jamais un second système de réglages) — affiche les vrais compteurs actives/éteintes/impossibles et renvoie vers le Centre Intelligence pour tout changement.\n4. `server/intelligences/provider.ts::verifierAcces()` : vérifié par appel réel au compte OpenAI de production (2026-09-26) que son budget de 16 jetons était insuffisant — gpt-5.5 (modèle réellement résolu ce jour) consomme déjà 10 à 18 jetons de raisonnement internes avant d'écrire « OK », ce qui faisait échouer ce test de bout en bout avec un vrai HTTP 400 OpenAI (« Could not finish the message because max_tokens... »), classé « Dégradé » avec ce message brut affiché tel quel — alors que les vraies conversations (budgets 900 à 4000) répondaient normalement. Porté à 64 jetons (marge ~3x sur le maximum observé, vérifié réellement suffisant).",
    pourquoi:
      "Chaque symptôme décrit par le PDG avait une cause réelle et distincte, jamais un simple ressenti : le menu à plat existait bien (20 boutons en `flex-wrap`), le débordement de la saisie était mesurable (le fil n'était pas borné), la carte de réglages dans Compte n'existait tout simplement pas encore, et le bandeau « Dégradé » provenait d'un vrai budget de jetons insuffisant sur l'appel de santé, jamais d'un état figé ou fabriqué. Vérifier chaque cause avant de coder a évité de masquer le symptôme (ex. cacher le message technique du bandeau) sans corriger la cause — règle constante de cet engagement.",
    ou: [
    ...
    lecon:
      "Vérifié visuellement en conditions réelles : serveur de développement démarré, compte PDG réel connecté (identifiants de seed), capture d'écran via Chromium (viewport mobile 390×844, proche du téléphone du PDG) avant ET après chaque correctif — pas seulement un typecheck. Cela a révélé un vrai bug introduit par ce lot lui-même : le texte que j'ai tapé dans la carte « Réglages » de Compte.tsx (accents, tiret cadratin, guillemets français) s'est retrouvé littéralement échappé (`\\u00e9`, `\\u2014`…) dans le texte JSX affiché au lieu d'être décodé, parce qu'un texte JSX enfant n'est pas une chaîne JS et n'interprète jamais ces séquences — contrairement à une chaîne entre guillemets ailleurs dans le même fichier. Corrigé en enveloppant ce texte dans une expression `{\"...\"}` (une vraie chaîne JS, où la même séquence se décode correctement). Repéré uniquement parce que la page a été rechargée dans un vrai navigateur après l'écriture du texte — un `tsc --noEmit` propre ne l'aurait jamais révélé, ce type d'erreur n'étant pas une erreur de type.\n\nTests : reasoning-effort-outils.test.ts 24/24, moderation.test.ts 16/16, fuite-fournisseurs.test.ts 45/45, api-externes.test.ts 29/29, boucle.test.ts 371/371 (tous inchangés, aucune régression) ; nouveau verifier-acces.test.ts 4/4 (budget de jetons envoyé = 64, jamais 16 ; le vrai HTTP 400 « max_tokens » observé reste honnêtement rapporté, jamais masqué). `check:providers`, `check:public-provider-leaks`, `check:intelligence-chat`, `check:naming`, `check:routers`, `check:routes`, `check:boutons` verts ; `npx tsc --noEmit` sans nouvelle erreur dans les fichiers touchés.\n\nLeçon générale : une modification purement visuelle (regrouper des onglets, ajouter un texte) reste soumise aux mêmes règles qu'un correctif serveur — jamais de bouton sans effet réel (pas de pouce, pas de partage, faute de moteur de note côté serveur), et une vérification visuelle réelle (navigateur, pas seulement le compilateur) est le seul moyen fiable de repérer un bug d'encodage qui ne casse ni les types ni les tests.",
    domaine: "moteurs",
  },
];
